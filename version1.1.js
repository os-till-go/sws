...
const boundry = [create_rectangle(10, 1200), create_rectangle(10, 1200), 
                create_rectangle(1200, 10), create_rectangle(1200, 10)];
...
function check_barri(player, barri){
    ...
    for(let i = 0; i < array_length(boundry); i = i + 1){
        if( gameobjects_overlap(boundry[i], player) ){
            return true;
        }
    }
    return false;
...
update_loop(game_state => {
    ...
    if( gameobjects_overlap(boundry[LEFT], player[cond])){
        update_position(player[cond], [new_position[0] + 1200 * movement_dist, new_position[1]]);
    }
    else if( gameobjects_overlap(boundry[RIGHT], player[cond])){
        update_position(player[cond], [new_position[0] - 1200 * movement_dist, new_position[1]]);
    }
    else if( gameobjects_overlap(boundry[UP], player[cond])){
        update_position(player[cond], [new_position[0], new_position[1] + 1200 * movement_dist]);
    }
