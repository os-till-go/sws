...
const rule_text = create_text("keys: p1:q w e t y / p2:[ ] / 1 2");

update_position(game_text, [100, 25]);
   ...
   if(! is_borderline(barri, query_position(player[cond]), 0)){
    ...
    for(let i = 0; i < array_length(swords); i = i + 1){
        if(gameobjects_overlap(swords[i], player2[cond2])){
            ...
            else if(is_towards_right(cond2)){
                const x = query_position(player2[cond2])[0];
                const y = query_position(player2[cond2])[1];
                update_position(player2[cond2], [-inf, -inf]);
                cond2 = sword_right;
    ...
    if(gameobjects_overlap(player2[cond2], player2[cond2])){
        if((cond2 === punch2_left || cond2 === punch_left)
            && query_position(player2[cond2])[0] < query_position(player2[cond2])[0]
            && math_abs( query_position(player2[cond2])[1] - query_position(player2[cond2])[1] ) < 30
        ){
    ...
    const point_xy2 = query_pointer_position();
    
    let temp2 = player;//seems useless
